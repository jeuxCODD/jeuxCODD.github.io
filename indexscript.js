 /*Le filtre de profanité est tiré de https://github.com/The-Best-Codes/bc-ProfanityBlock/tree/main
 Le code apres ligne 492 est le nôtre */
 // Profanity filter class definition
 class ContentFilterBadWord {
   constructor() {
     this.encodedCussWords = [      
      ...
      "bmlnZXI=",
      "bmlnZXJz",];
      //certains mots on été ajouté par nous
     this.evasionPatterns = [
       ...
       { pattern: /é/gi, replacement: "e" },
       { pattern: /è/gi, replacement: "e" },
       { pattern: /ê/gi, replacement: "e" },
       { pattern: /3/gi, replacement: "e" },
       { pattern: /µ/gi, replacement: "u" },
       { pattern: /\|/gi, replacement: "i" },
     ];
     //Nous avons ajouté les 6 derniers remplacements
   }
   ...
   containsBadWords(text) {
     let normalized = this.normalize(text.toLowerCase());
     normalized = normalized.replace(/[-_.]/g, "");
     const badWords = this.encodedCussWords.map(e => this.decodeBase64(e).toLowerCase());
 ...
 function nomCheck() {
     ...
     const nomValue = inputEl.value.trim();

     // vérifie longeure
     if (nomValue.length === 0) {
  alert("Entrer un nom pour votre morue.");
  return;
     }
  ...
  return;
     ...
     // vérifier retours
     if (filter.containsBadWords(nomValue)) {
  alert("Le nom contient des mots interdits. Veuillez en choisir un autre.");
  return;
     }
